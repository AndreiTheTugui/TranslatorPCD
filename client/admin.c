...
#define BLOCKED_USERS_FILE "/tmp/blocked_users.txt" /* path-ul catre fisierul cu utilizatori blocati */
...
void helpCommand() {
    ...
    fprintf(stdout, "  add_blocked_user <username> - Adauga un utilizator la lista blocata\n");
    fprintf(stdout, "  remove_blocked_user <username> - Sterge un utilizator din lista blocata\n");
...
void addBlockedUser(const char* username) {
    FILE* file = fopen(BLOCKED_USERS_FILE, "a");
    if (file == NULL) {
        perror("Eroare la adaugarea utilizatorului blocat");
        return;
    }
    fprintf(file, "%s\n", username);
    fclose(file);
    fprintf(stdout, "Utilizatorul blocat a fost adaugat cu succes: %s\n", username);
}

void removeBlockedUser(const char* username) {
    FILE* tempFile = fopen("/tmp/blocked_users_temp.txt", "w");
    if (tempFile == NULL) {
        perror("Eroare la crearea fisierului temporar pentru utilizatori blocati");
        return;
    }

    FILE* file = fopen(BLOCKED_USERS_FILE, "r");
    if (file == NULL) {
        perror("Eroare la deschiderea fisierului cu utilizatori blocati");
        fclose(tempFile);
        return;
    }
    ...
    if (found) {
        remove(BLOCKED_USERS_FILE);
        rename("/tmp/blocked_users_temp.txt", BLOCKED_USERS_FILE);
        fprintf(stdout, "Utilizatorul blocat a fost sters cu succes: %s\n", username);
    } else {
        remove("/tmp/blocked_users_temp.txt");
        fprintf(stdout, "Utilizatorul blocat nu a fost gasit: %s\n", username);
    }
}
...
int main() {
    ...
    for (;;) {
        ...
        if (strncmp(command, "add_language ", strlen("add_language ")) == 0) {
            ...
            showLanguages();
        } else if (strncmp(command, "add_blocked_user ", strlen("add_blocked_user ")) == 0) {
            char *username = command + strlen("add_blocked_user ");
            addBlockedUser(username);
            // Send command to server if needed
            send(sock, command, strlen(command), 0);
        } else if (strncmp(command, "remove_blocked_user ", strlen("remove_blocked_user ")) == 0) {
            char *username = command + strlen("remove_blocked_user ");
